...
def main(config: DiffusionPtqRunConfig, logging_level: int = tools.logging.DEBUG) -> DiffusionPipeline:
    ...
    if not config.skip_gen:
        logger.info("\n\nGENIMAGEHERE: Generating images from prompts")
        ...
        prompts = [
            ...
            "a guy looking to the side and smiling", "Two people using an interactive gaming system while a person observes them from a couch.",
        ]
        ...
        for prompt in prompts:
            ...
            try:
                # Generate image from the prompt using the pipeline
                image = pipeline(prompt).images[0]

                # Save the generated image
                output_path = os.path.join(config.output.running_job_dirpath, f"generated_image_{prompts.index(prompt)}.png")
